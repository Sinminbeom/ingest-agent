...
@router.post("/batch/{batch_public_id}", summary="배치 파일 업로드 시작", status_code=202)
def upload(
    ...
    ingest_agent.upload(str(ctx.tenant_public_id), str(batch_public_id), str(ctx.claims.member_public_id))
    return ApiResponse.ok(IngestStartResponse(batch_public_id=batch_public_id), status=202)
